...
var replaceNeo4jAtomWithMeteor = function (document) {
//  if (document instanceof MongoDB.Binary) {
//    var buffer = document.value(true);
//    return new Uint8Array(buffer);
//  }
//  if (document instanceof MongoDB.ObjectID) {
//    return new Meteor.Neo4jCollection.ObjectID(document.toHexString());
//  }
...
var replaceMeteorAtomWithNeo4j = function (document) {
...
//  }
//  if (document instanceof Meteor.Neo4jCollection.ObjectID) {
...
Neo4jObserver = function (watcher, observer) {
  var self = this;
  ...
  var listener = function (key, message) {
    ...
    if (!methodName) {
      Meteor._debug("Neo4jConnection::observe: Unknown message: " + message);
    } else {
...
Neo4jObserver.prototype.stop = function () {
  var self = this;
  self._watcher.removeListener(self._listener);
};

Neo4jConnection = function (url, options) {
  var self = this;
  options = options || {};
  ...
  var neo4jOptions = {}; // {db: {safe: true}, server: {}, replSet: {}};
...
//  }
  ...
  var client = self._client = new Neo4jClient(url, neo4jOptions);
  self._watcher = new Neo4jWatcher(url);
...
function checkConfig(client, fix) {
  ...
  if (missing) {
    if (fix) {
      ...
      throw new Error("You must configure notify-keyspace-events for Meteor (or launch with neo4j_CONFIGURE_KEYSPACE_NOTIFICATIONS=1).  Current config=" + config + " missing=" + missing);
    }
  }
};

Neo4jConnection.prototype.close = function() {
  var self = this;
...
Neo4jConnection.prototype._withDb = function (callback) {
  var self = this;
  if (self._client) {
...
Neo4jConnection.prototype._getCollection = function (collectionName) {
  var self = this;
...
Neo4jConnection.prototype._createCappedCollection = function (collectionName,
                                                              byteSize) {
  var self = this;
  var future = new Future();
  self._withDb(function (db) {
...
Neo4jConnection.prototype._maybeBeginWrite = function () {
  var self = this;
...
Neo4jConnection.prototype._onFailover = function (callback) {
...
Neo4jConnection.prototype._insert = function (collection_name, document,
                                              callback) {
  var self = this;
  ...
  try {
    ...
    collection.insert(replaceTypes(document, replaceMeteorAtomWithNeo4j),
                      {safe: true}, callback);
  } catch (e) {
    write.committed();
    throw e;
...
// fence.
Neo4jConnection.prototype._refresh = function (collectionName, selector) {
  var self = this;
...
Neo4jConnection.prototype._remove = function (collection_name, selector,
                                              callback) {
  var self = this;
  ...
  try {
    var collection = self._getCollection(collection_name);
    collection.remove(replaceTypes(selector, replaceMeteorAtomWithNeo4j),
                      {safe: true}, callback);
  } catch (e) {
    write.committed();
    throw e;
...
Neo4jConnection.prototype._dropCollection = function (collectionName, cb) {
  var self = this;
...
Neo4jConnection.prototype._update = function (collection_name, selector, mod,
                                              options, callback) {
  var self = this;
  ...
  try {
    ...
    var mongoSelector = replaceTypes(selector, replaceMeteorAtomWithNeo4j);
    var mongoMod = replaceTypes(mod, replaceMeteorAtomWithNeo4j);
...
// exposed for testing
Neo4jConnection._isCannotChangeIdError = function (err) {
...
var simulateUpsertWithInsertedId = function (collection, selector, mod,
  ...
  if (isModify) {
    // We've already run replaceTypes/replaceMeteorAtomWithNeo4j on
  ...
  var doConditionalInsert = function () {
    var replacementWithId = _.extend(
      replaceTypes({_id: insertedId}, replaceMeteorAtomWithNeo4j),
      newDoc);
    collection.update(selector, replacementWithId, mongoOptsForInsert,
                      bindEnvironmentForWrite(function (err, result) {
                        if (err) {
                          ...
                          if (Neo4jConnection._isCannotChangeIdError(err)) {
                            doUpdate();
                          } else {
                            callback(err);
                          }
...
_.each(["insert", "update", "remove", "dropCollection"], function (method) {
  Neo4jConnection.prototype[method] = function (/* arguments */) {
    throw new Error("not part of neo4j api");
  };
});

_.each(NEO4J_COMMANDS_LOCAL, function (method) {
  Neo4jConnection.prototype[method] = function (/* arguments */) {
    var self = this;
...
_.each(["set", "setex", "append", "del",
        ...
        "flushall"].concat(NEO4J_COMMANDS_HASH), function (method) {
  ...
  Neo4jConnection.prototype["_" + method] = function (key /*, arguments */) {
    var self = this;
...
Neo4jConnection.prototype.getIndexedNodes = function (collectionName, pattern) {
  var self = this;
    console.log("TODO inside SynchronousCursor 3.5");

  return new Cursor(
    self, new CursorDescription(collectionName, pattern));
};

Neo4jConnection.prototype._observe = function (observer) {
  var self = this;
  return new Neo4jObserver(self._watcher, observer);
};


// We'll actually design an index API later. For now, we just pass through to
// Mongo's, but make it synchronous.
Neo4jConnection.prototype._ensureIndex = function (collectionName, index,
                                                   options) {
  var self = this;
...
Neo4jConnection.prototype._dropIndex = function (collectionName, index) {
  var self = this;
...
// documented Meteor.Neo4jCollection cursor API.  It wraps a CursorDescription and a
...
//   - KeyspaceNotificationObserveDriver uses neo4j keyspace notifications to
...
Cursor.prototype._publishCursor = function (sub) {
  var self = this;
  var collection = "neo4j"; // XXX don't hard-code this
  return Meteor.Neo4jCollection._publishCursor(self, sub, collection);
};
...
//    self._cursorDescription, ordered, callbacks);
  console.log("TODO inside observeChanges 1");
  try {
...
Neo4jConnection.prototype._createSynchronousCursor = function(
    cursorDescription, options) {
  var self = this;

  console.log("TODO inside SynchronousCursor 1");
...
//    replaceTypes(cursorDescription.selector, replaceMeteorAtomWithNeo4j),
//    cursorOptions.fields, mongoOptions);
//
//  return new SynchronousCursor(dbCursor, cursorDescription, options);


  console.log("TODO inside SynchronousCursor 2");
  var future = new Future;
  self._withDb(function (db) {
    console.log("TODO inside SynchronousCursor 3");
    db.getIndexedNodes(pattern, future.resolver());
...
Neo4jConnection.prototype._observeChanges = function (cursorDescription, callbacks) {
  var self = this;
...
//  }
  ...
  console.log("TODO inside observeChanges 2");
  ...
  console.log("TODO inside observeChanges 3");
...
//    var driverClass = KeyspaceNotificationObserveDriver;
  ...
  console.log("TODO inside observeChanges 4");
...
//     new Neo4jInternals.MongoTimestamp(0, 0) (which is initialized to the
...
Neo4jConnection.prototype._observeChangesTailable = function (
...
//Neo4jInternals.MongoTimestamp = MongoDB.Timestamp;

Neo4jInternals.Connection = Neo4jConnection;
